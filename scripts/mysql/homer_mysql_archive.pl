...
my $newtables = $CONFIG->{"MYSQL_ARCHIVE"}{"newtables"};
...
foreach my $table (keys %{ $CONFIG->{"DATA_TABLE_ARCHIVE"} }) {

    my $rotate = $CONFIG->{'DATA_TABLE_ARCHIVE'}{$table};
    my $partstep = $CONFIG->{'DATA_TABLE_STEP'}{$table};
    $newparts = $CONFIG->{'MYSQL'}{'newtables'};
    $maxparts = $CONFIG->{'DATA_TABLE_ARCHIVE'}{$table} + $newparts;
    ...
    if(defined $CONFIG->{'DATA_TABLE_TYPE_TIMESTAMP'}{$table})
    {
        ...
        for(my $y = 0 - $CONFIG->{'DATA_TABLE_ARCHIVE'}{$table}; $y<($newtables+1); $y++) {
        ...
        my $rotation_horizon = $CONFIG->{"DATA_TABLE_ARCHIVE"}{$table};
    ...
    #Rtcp, Logs, Reports tables
        ...
        $newparts *= $coof;
        #Now
        new_partition_table($db, $CONFIG->{"MYSQL_ARCHIVE"}{"db_data"}, $table, $mystep, $partstep, $maxparts, $newparts);
    }
}
...
$maxparts = 1;
$newparts = 1;
foreach my $table (keys %{ $CONFIG->{"STATS_TABLE_ARCHIVE"} }) {

    $newparts = $CONFIG->{'MYSQL'}{'newtables'};
    $maxparts = $CONFIG->{'STATS_TABLE_ARCHIVE'}{$table} + $newparts;
    ...
    new_partition_table($db, $CONFIG->{"MYSQL_ARCHIVE"}{"db_stats"}, $table, $mystep, $partstep, $maxparts, $newparts);
}
...
sub db_connect {
    ...
    my $db_name = shift;
    my $dbistring = "";
    if($CONFIG->{"MYSQL_ARCHIVE"}{"usesocket"}) {
    	$dbistring = "DBI:mysql:database=".$CONFIG->{"MYSQL_ARCHIVE"}{$db_name}.";mysql_socket=".$CONFIG->{"MYSQL_ARCHIVE"}{"socket"}
    } else {
    	$dbistring = "DBI:mysql:".$CONFIG->{"MYSQL_ARCHIVE"}{$db_name}.":".$CONFIG->{"MYSQL_ARCHIVE"}{"host"}.":".$CONFIG->{"MYSQL_ARCHIVE"}{"port"}
    }
    my $db = DBI->connect($dbistring, $CONFIG->{"MYSQL_ARCHIVE"}{"user"}, $CONFIG->{"MYSQL_ARCHIVE"}{"password"});
    return $db;
}
...
sub new_partition_table {
    ...
    ##my $query = "SHOW TABLE STATUS FROM ".$CONFIG{"MYSQL_ARCHIVE"}{"db_stats"}. " WHERE Name='".$table."'";
